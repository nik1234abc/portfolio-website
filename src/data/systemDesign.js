export const systemDesign = {
  ...
  questions: [
    ...
    // ─── BATCH 6: Q26–30 – Microservices + Event-Driven Design ───────────────
    {
      ...
      simpleAnswer: "Use REST (sync) when you need an immediate response. Use Kafka (async) when you want to decouple services, handle high throughput, or the caller doesn't need to wait for the result.",
      explanation: "REST: OrderService calls PaymentService and waits — needs to know if payment succeeded before confirming the order. Kafka: After payment, OrderService publishes OrderPaid event — InventoryService, NotificationService, InvoiceService all consume it independently. They don't need to respond to OrderService. Rule of thumb: if the caller needs the result to continue → REST. If the caller just needs to notify others → Kafka.",
      example: "REST: POST /payments — OrderService waits for 200 OK before confirming order to user. Kafka: After order confirmed, publish OrderConfirmed event — NotificationService sends email, InventoryService reduces stock, InvoiceService generates PDF — all async, all independent.",
      followUps: [
        { question: "What are the risks of using REST between microservices?", answer: "Tight temporal coupling — if PaymentService is down, OrderService fails. Cascading failures. Latency adds up across multiple hops. Use circuit breakers (Resilience4j) to handle downstream failures gracefully." },
        { question: "Can you mix REST and Kafka in the same flow?", answer: "Yes — this is common. Use REST for the critical synchronous path (payment confirmation) and Kafka for downstream async processing (notifications, analytics, invoicing). Best of both worlds." }
      ],
      keyPoints: ["REST: immediate response needed, both services must be up", "Kafka: fire-and-forget, decoupled, high throughput", "Mix both in the same system based on use case", "Kafka for fan-out to multiple consumers"]
    },
    {
      id: 27,
      category: "System Design",
    ...
    },
    {
      ...
      simpleAnswer: "Use SQL for structured data with relationships and ACID transactions. Use NoSQL for unstructured/semi-structured data, high write throughput, flexible schema, or horizontal scale.",
      explanation: "SQL (PostgreSQL, MySQL): ACID transactions, complex joins, strong consistency, fixed schema. Best for: financial data, user accounts, order management — anything needing consistency and relationships. NoSQL: 1) Document (MongoDB) — flexible schema, nested data (product catalogs, user profiles). 2) Key-Value (Redis) — ultra-fast reads/writes (caching, sessions). 3) Wide-Column (Cassandra) — high write throughput, time-series data (logs, IoT). 4) Graph (Neo4j) — relationship-heavy data (social networks).",
      example: "E-commerce: SQL for orders and payments (ACID needed). MongoDB for product catalog (flexible attributes per category). Redis for cart and sessions (fast key-value). Cassandra for order event history (high write, time-series). Elasticsearch for product search.",
      followUps: [
        { question: "Can you use both SQL and NoSQL in the same system?", answer: "Yes — polyglot persistence. Each service picks the best DB for its needs. OrderService uses PostgreSQL, CatalogService uses MongoDB, SessionService uses Redis. This is a microservices best practice." },
      ...
      keyPoints: ["SQL: ACID, relationships, structured data", "NoSQL: scale, flexibility, high throughput", "Polyglot persistence: different DB per service", "Choose based on data shape and consistency requirements"]
    },
    {
      id: 33,
      category: "System Design",
    ...
    },
    {
      id: 39,
      category: "System Design",
      topic: "11. Spring Boot Configuration",
    ...
    },
    {
      id: 40,
      category: "System Design",
      topic: "12. Spring Boot Internals",
    ...
    // ─── BATCH 10: Q46–50 – Real Project Deep Dive ───────────────────────────
    {
      id: 46,
      category: "System Design",
      topic: "9. Real Project Deep Dive",
    ...
    },
    {
      id: 54,
      category: "System Design",
      topic: "11. Spring Boot + POM Connection",
    ...
    // ─── BATCH 12: Q56–60 – Config + Embedded Server + Internal Flow ─────────
    {
      id: 56,
      category: "System Design",
      topic: "12. Configuration Deep Dive",
    ...
    },
    {
      id: 58,
      category: "System Design",
      topic: "13. Embedded Server & Runtime",
    ...
    },
    {
      id: 60,
      category: "System Design",
      topic: "14. Internal Flow Details",
